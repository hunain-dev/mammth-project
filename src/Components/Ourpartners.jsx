...
const Ourpartners = () => {
  ...
  return (
    <div className="w-full text-center lg:py-10">
      <h3 className="SuisseIntl font-[550] md:text-3xl lg:text-[1.4vw] text-2xl mb-6">
        Partners that trust our work
      </h3>

      <div className="grid lg:grid-cols-4 grid-cols-2 md:grid-cols-3 gap-5 transition-all duration-700 ease-in-out">
        {sets[currentSet].map((img, index) => (
          <div key={index} className="flex items-center justify-center p-4">
            <img
              src={img}
              alt="partner"
              className="lg:h-[100px] h-25 md:h-22 object-contain transition-all duration-700 ease-in-out"
            />
          </div>
        ))}
      </div>
